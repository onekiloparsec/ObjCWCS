...

{
	...
	if (ranges != NULL) {
		...
		for (i = 0; i < ns; i++ ) {
			...
			dec2str__ (decstr, 32, gdec[i], nddec);
			}
	...
	/* Find stars specified by location */
		...
		if (ns > 0) {
			...
			dec2str__ (decstr, 32, cdeco, nddec);
			}
		    if (tabout)
			...
			dec2str__ (decstr, 32, gdec[0], nddec);
			}
		    if (tabout)
			printf ("	%s	%s	%s",
			        numstr, rastr, decstr);
...
double	*cdeco;		/* Output search center latitude/declination */
{
	...
	*ddec = *drad;
	...
	if (syscoor == WCS_XY) {
	    ...
	    dec2str__ (rstr, 32, *dra, 2); 
	    dec2str__ (dstr, 32, *ddec, 2); 
	    dec2str__ (dstro, 32, *drad, 2); 
	    dec2str__ (dstri, 32, *dradi, 2); 
	    }
	if (*drad == 0.0)
...
int	degout;		/* Ouput in degrees if 1 */
{
	...
	dec2str__ (decstr, 32, cdec, nddec);
	}
	...
	if (drad != 0.0) {
	    dec2str__ (decstr, 32, drad, 1);
	    if (dradi > 0.0) {
		dec2str__ (rastr, 32, dradi, 1);
	    ...
	    dec2str__ (rastr, 32, dra, 1);
	    dec2str__ (decstr, 32, ddec, 1);
	    printf (" +- %s %s", rastr, decstr);
	    }
	}
    if (classd == 0)
...
char *parstring;
{
    ...
    *parequal = (char) 0;
	...
	if (strchr (parvalue,':'))
	    rad0 = 3600.0 * str2dec__ (parvalue);
	...
	if (strchr (parvalue,':'))	/* hours:minutes:seconds */
	    rad1 = 3600.0 * str2dec__ (parvalue);
	else if (isnum (parvalue))
	    rad1 = atof (parvalue);
	else {
	...
	if (strchr (parvalue,':'))
	    ddec0 = 3600.0 * str2dec__ (parvalue);
	else
	    ddec0 = atof (parvalue);
	if (dra0 <= 0.0)
	    dra0 = ddec0;
 ...
 * Jun 24 1998	Add string lengths to ra2str() and dec2str__() calls
