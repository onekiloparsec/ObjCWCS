...

{
	...
	for (s = 0; s < ns; s++) {
		...
		if (mxy[igs] > 0.0) {
			...
			if (debug) {
			    ...
			    dec2str__ (decstr, 31, gdec[ibg[nmatch]], 2);
	...
	dec2str__ (decstr, 31, wcs->yref, 2);
	fprintf (stderr,"\nfirst solution:\n");
	if (vfit[6] > -1)
	    fprintf (stderr," cra= %s cdec= %s cd = %9.7f,%9.7f,%9.7f,%9.7f ",
	...
	*sy = ty;
	...
	nextline = line;
		...
		*lastchar = (char) 0;
		...
		dec2str__ (decstr, 31, tdec[nmatch], 2);
...

{
	...
	for (i = 0; i < nmatch; i++) {
		...
		gbra[i] = gbra[i] - 360.0;
	    if (debug) {
		dec2str__ (rastr, 31, gbra[i], 3);
		dec2str__ (decstr, 31, gbdec[i], 2);
...
#define	PDUMP
	...
	if (vfit[2] > -1)
	    dec2str__ (decstr, 16, p[i][vfit[2]]+yref_p, 2);
	else
	    dec2str__ (decstr, 16, wcsf->yref, 2);
	if (vfit[6] > -1) {
	    cd[0] = p[i][vfit[3]];
	    cd[1] = p[i][vfit[4]];
	    cd[2] = p[i][vfit[5]];
...
#define	PDUMP
	...
	if (vfit[1] > -1)
	    ...
	    ra2str (rastr, 31, wcsf->xref, 3);
	if (vfit[2] > -1)
	    dec2str__ (decstr, 31, p[i][vfit[2]]+yref_p, 2);
	else
	    dec2str__ (decstr, 31, wcsf->yref, 2);
	if (vfit[6] > -1) {
	    cd[0] = p[i][vfit[3]];
	    cd[1] = p[i][vfit[4]];
	    cd[2] = p[i][vfit[5]];
